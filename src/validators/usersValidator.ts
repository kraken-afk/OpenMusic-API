...
import { type UserCreation, type ServerResponse } from '../app.d'
...
export async function validateUserCreation (req: Request, h: ResponseToolkit) {
  ...
  const isUsernameAlreadyExist = typeof payload?.username === 'string' ? await UsersModel.isAlreadyExist(payload.username) : true

  if ((error == null) && !isUsernameAlreadyExist) { return h.continue } else {
    const message = (error != null) ? error.details.map(({ message }) => message) : []

    if (isUsernameAlreadyExist && typeof payload?.username === 'string') { message.push('"username" already exist') }
