...
export interface CollaborationsData { collaborationsId: string }
...
export default abstract class CollaborationsModel {
  static async create (
    ...
    const verificationUser = (() => {
      if (Array.isArray(userIds) && Array.isArray(user)) {
        const illegalUser = userIds.filter(
          (e) => !user.map((e) => e.id).includes(e)
        )
        return () => {
        ...
        return () => {
          if (user == null) { throw new NotFoundError(`User with id: ${userIds} doesn't exist`) }
        }
      }
    })()
  ...
  static async removeUser (ownerId: string, playlistId: string, userId: string) {
    ...
    if (collaborationPlaylist == null) { throw new InvariantError('Something went Error') }

    if (!collaborationPlaylist.userIds.includes(userId)) { throw new NotFoundError(`User with id: ${userId} doesn't exist`) }
