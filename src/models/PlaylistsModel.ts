...
import { Op, type NonNullFindOptions } from 'sequelize'
...
export default abstract class PlaylistsModel {
  static async getPlaylist (
    ...
    if (!playlist) { throw new NotFoundError(`Playlist with id: ${id} doesn't exist`) }
  ...
  static async create ({
    ...
    try {
      ...
      if ((await Users.findByPk(owner)) === null) { throw new InvariantError(`Couldn't found user by id: ${id}`) }
  ...
  static async getAll ({
    ...
    if (user === null) { throw new Error(`Couldn't find username of id: ${owner}`) }
  ...
  static async remove (playlistId: string, ownerId: string) {
    ...
    if (playlist.owner !== ownerId) { throw new ForbiddenError('Forbiden resource') }

    if (collaborationPlaylist != null) { await collaborations.destroy({ where: { id: collaborationPlaylist.id } }) }

    const affectedRow = await Playlists.destroy({ where: { id: playlistId } })

    if (!affectedRow) { throw new NotFoundError(`Playlist with id: ${playlistId} doesn't exist`) }
  }
  ...
  static async addSong (
    ...
    if (playlist == null) { throw new InvariantError(`Playlist with id: ${playlistId} doesn't exist`) }

    if (playlist.owner !== ownerId && (collaborationPlaylist == null)) { throw new ForbiddenError('Forbiden resource') }

    if ((await Songs.findByPk(songId)) == null) { throw new NotFoundError(`Song with id: ${songId} doesn't exist`) }
